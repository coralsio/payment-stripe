    ...
    public function getMoney($parameterName = 'amount')
    {
        ...
        if ($amount !== null) {
            ...
            if (!$this->negativeAmountAllowed && $money->isNegative()) {
                throw new InvalidRequestException(trans('Stripe::exception.negative_not_allowed'));
            }

            // Check for a zero amount.
            if (!$this->zeroAmountAllowed && $money->isZero()) {
                throw new InvalidRequestException(trans('Stripe::exception.zero_amount_not_allowed'));
            }
